...
public final class VClientImpl extends IVClient.Stub {
    ...
    private void bindApplicationNoCheck(String packageName, String processName, ConditionVariable lock) {
        ...
        try {
            ...
            /**
             * andhook  add by mt 18/12/05
             * 当前插件应用为微信应用时，动态加载hook.jar包
             */
            if(isWechatApp(packageName)){
                Log.e("HookDemo","isWeiXinApp:::");
                loadHookDex(context);
            }
    ...
    private static int isWechatApp = -1;
    public boolean isWechatApp(String packageName){
        if(isWechatApp == 1){
            return  true;
        }else if(isWechatApp == 0){
            return  false;
        }else{
    ...
    private void loadHookDex(Context context){
        Log.e("HookWechat" , "loadHookDex");
        if(VirtualCore.get().getExtendDexClassLoader() != null){
            return ;
        }
        File check = VirtualCore.get().getContext().getDir("HookDex", Context.MODE_PRIVATE);
        String path = new File(check , "hook.dex.jar").getAbsolutePath();
        Log.e("HookWechat" , "loadHookDex:path:" + path);
        ...
        try {
            ...
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT && Build.VERSION.SDK_INT <= Build.VERSION_CODES.LOLLIPOP_MR1){
                File[] nativeLibrarys = Reflect.on(Reflect.on(VClientImpl.class.getClassLoader()).get("pathList")).get("nativeLibraryDirectories");
                if(nativeLibrarys != null && nativeLibrarys.length > 0){
                    for (File nativeLibrary : nativeLibrarys){
                        if(nativeLibrary != null && nativeLibrary.getAbsolutePath().contains(VirtualCore.get().getHostPkg())){
                            libSearchPath = nativeLibrary.getAbsolutePath();
                            break;
                        }
                    }
            ...
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && Build.VERSION.SDK_INT <= 28){
                List<File> nativeLibrarys = Reflect.on(Reflect.on(VClientImpl.class.getClassLoader()).get("pathList")).get("nativeLibraryDirectories");
                if(nativeLibrarys != null && nativeLibrarys.size()>0){
                    for(File nativeLibrary : nativeLibrarys){
                        if(nativeLibrary != null && nativeLibrary.getAbsolutePath().contains(VirtualCore.get().getHostPkg())){
                            libSearchPath = nativeLibrary.getAbsolutePath();
                            break;
            ...
            //String dexCache = VirtualCore.get().getContext().getDir("HookDex" , Context.MODE_PRIVATE).getAbsolutePath();
            String dexCache = null;
            DexClassLoader dexClassLoader = new DexClassLoader(path, dexCache, libSearchPath, VClientImpl.class.getClassLoader());
            //保存dexClassLoader
            VirtualCore.get().setExtendDexClassLoader(dexClassLoader);

            Class clazz = dexClassLoader.loadClass("wechathook.HookWechat");
            Method getInstance = clazz.getDeclaredMethod("getInstance");
            Object hookWechat = getInstance.invoke(null);
            if(hookWechat != null){
                Log.e("HookWechat", "loadHookDex:::run");
                Reflect.on(hookWechat).call("run", context);
            }
